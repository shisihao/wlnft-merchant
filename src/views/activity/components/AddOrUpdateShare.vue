  ...
  <div>
    ...
    >
      <el-form ref="form" :model="form" label-width="80px" :rules="rules">
        ...
        <el-card shadow="never" class="box-card">
          ...
          <el-form-item label="藏品/盲盒" prop="reward">
            <div v-for="(item, index) in form.reward.goods" :key="item.goods_id" class="box-wrapper">
              ...
              <i v-if="oldfrom.id>0&&!oldfrom.reward.stock>0" class="el-icon-delete del-good" @click="onDelGood(0, index)" />
            </div>
            <div v-for="(item, index) in form.reward.box" :key="item.box_id" class="box-wrapper">
              ...
              <i v-if="oldfrom.id>0&&!oldfrom.reward.stock>0" class="el-icon-delete del-good" @click="onDelGood(1, index)" />
            </div>
        ...
        <el-form-item label="分享链接" prop="extend.url">
          <el-input v-model="form.extend.url" placeholder="请输入分享链接" />
        </el-form-item>
        <el-form-item label="活动描述" prop="extend.content">
          <el-input v-model="form.extend.content" type="textarea" placeholder="请输入活动描述" :rows="4" />
        </el-form-item>
...
export default {
  ...
  computed: {
    ...
    disabledStock() {
      if (this.oldfrom.id && this.oldfrom.reward.stock > 0 && (this.oldfrom.reward.box.length > 0 || this.oldfrom.reward.goods.length > 0)) {
        return true
      }
      return false
    }
  ...
  methods: {
    ...
    async getShareDetail(id) {
      const { data } = await shareDetail(id)
      return data
    },
    onChangeGood(id) {
