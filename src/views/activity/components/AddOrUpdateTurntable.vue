        ...
        <div class="reward-wrap">
          ...
          <div class="reward-list-wrap">
            <div v-for="(item, index) in form.reward" :key="index" class="reward-list">
              ...
              <span class="item">奖励数量 {{ item.reward_num }}</span>
              <span class="item">库存 {{ item.stock }}</span>
              <i v-if="disabledBtn" class="el-icon-delete" @click="deleteReward(index)" />
            </div>
          </div>
          <div v-if="!form.id||disabledBtn">
            <el-button v-if="disabledBtn&&!rewardFlag || !form.id&&rewardFlag" icon="el-icon-plus" @click="resetAddBtn()" />
            <el-card v-else shadow="never" class="box-card">
              ...
              <el-form-item label="icon">
                ...
                <div class="notice">建议icon尺寸：96 x 96</div>
              </el-form-item>
              <el-form-item required label="奖励数量">
                <el-input-number v-model="selectRewardList.reward_num" :min="1" :precision="0" :max="maxNumber" @change="rewardNumChange" />
              </el-form-item>
              <el-form-item required label="库存">
                <el-input-number v-model="selectRewardList.stock" :min="0" :precision="0" :max="maxNumber" @change="stockChange" />
              </el-form-item>
              <el-form-item>
  ...
  methods: {
    ...
    onFormSubmit() {
      this.$refs['form'].validate((valid) => {
        if (valid && this.form.reward.length) {
          this.btnLoading = true
          addOrUpdateTurntable(this.form).then()
            .then(({ msg }) => {
