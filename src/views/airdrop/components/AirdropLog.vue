<template>
  <el-dialog
    top="30px"
    width="1000px"
    :title="form.status === 1 ? '空投成功记录' : '空投失败记录'"
...
import { exportOrder, airdropLogs, failedLogs } from '@/api/airdrop'
...
export default {
  ...
  computed: {
    imageUrl() {
      return (item) => {
        return item.image || item.goods?.images[0]
      }
    }
  },
  methods: {
    ...
    getList(page = this.pages.current, loading = true) {
      ...
      this.search = { goods_id: this.goods_id, page, limit: this.pages.limit }
      const api =
        this.form.status === 1
