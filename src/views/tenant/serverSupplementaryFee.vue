    ...
    <el-table v-loading="loading" border :data="list">
      ...
      <el-table-column header-align="center" label="账户信息">
        <template slot-scope="{ row }">
          ...
          <div>开户银行：{{ row.bank_info.bank }}</div>
          <div>帐号：{{ row.bank_info.account_number }}</div>
        </template>
      </el-table-column>
      <el-table-column width="120" label="汇款凭证" align="center">
        <template slot-scope="{ row }">
          <el-image
            v-if="row.credential"
            style="width: 60px"
...
export default {
  ...
  methods: {
    ...
    handleOprate(row) {
      ...
      this.$nextTick(() => {
        this.$refs.payFee && this.$refs.payFee.init(row, 'supplementary')
      })
    }
  }
}
